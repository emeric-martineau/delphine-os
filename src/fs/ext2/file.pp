   ...
   begin
      ...
      begin
			block := ext2_new_block(inode);
	 		if (longint(block) <= 0) then goto end_write;
			{$IFDEF DEBUG_EXT2_FILE_WRITE}
...
{   else if (block <= EXT2_NDIR_BLOCKS + (blocksize div 4)) then}
   ...
   begin
      ...
      begin
			...
			bh := alloc_buffer_head(major, minor, new_block, blocksize);
			if (bh = NIL) then
			begin
				print_bochs('ext2_set_real_block: alloc_buffer_head() failed\n', []);
				exit;
			end;

			lock_buffer(bh);
			memset(bh^.data, 0, blocksize);
			bh^.state := bh^.state or BH_Uptodate;
			unlock_buffer(bh);
		...
		else   { <=> inode^.ext2_i.data[EXT2_IND_BLOCK] <> 0 }
      begin
...
{   else if (block <= (EXT2_NDIR_BLOCKS + (blocksize div 4) + ((blocksize div 4) * (blocksize div 4)))) then}
   ...
   begin
		...
		else   { <=> inode^.ext2_i.data[EXT2_DIND_BLOCK] <> 0 }
		begin
			bh := bread(major, minor, inode^.ext2_i.data[EXT2_DIND_BLOCK], blocksize);
	 		if (bh = NIL) then
	 		begin
	    		print_bochs('ext2_set_real_block: cannot read block %d (step 2)\n',
	            			[inode^.ext2_i.data[EXT2_DIND_BLOCK]]);
	    		exit;
	 		end;
		end;

		{ Here, bh is set and ok }
      ...
      begin
			...
			bh2 := alloc_buffer_head(major, minor, new_block, blocksize);
			if (bh2 = NIL) then
			begin
				print_bochs('ext2_set_real_block: alloc_buffer_head() failed\n', []);
				brelse(bh);
			...
			lock_buffer(bh2);
			memset(bh2^.data, 0, blocksize);
			bh2^.state := bh2^.state or BH_Uptodate;
			unlock_buffer(bh2);

			buffer[(block - (EXT2_NDIR_BLOCKS + (blocksize div 4))) div (blocksize div 4)] := new_block;
			mark_buffer_dirty(bh);
		...
		buffer[((block - (EXT2_NDIR_BLOCKS + (blocksize div 4))) mod (blocksize div 4))] := real_block;
