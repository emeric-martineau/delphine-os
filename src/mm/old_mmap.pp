...
begin
   ...
   if (longint(start) mod 4096 <> 0) or (len = 0) then
	begin
		print_bochs('sys_munmap (%d): start=%h (page-aligned)\n', [current^.pid, start, len]);
		exit;
	end;
...
begin
   ...
   if (flags and MREMAP_FIXED) = MREMAP_FIXED then
       print_bochs('sys_mremap (%d): MREMAP_FIXED\n', [current^.pid]);
   ...
   begin
      ...
      begin
         new_page := get_free_page();
	 		if (new_page = NIL) then
	 		begin
	    		print_bochs('sys_mremap (%d): no enough memory to add %d pages\n', [current^.pid, (new_len - old_len) div 4096]);
	    		goto out;
	 		end;
   ...
   begin
      ...
      begin
			new_page := get_free_page();
	 		if (new_page = NIL) then
	 		begin
	    		print_bochs('sys_mremap (%d): not enoug memoty to reallocate\n', [current^.pid]);
	    		goto out;
	 		end;
	 		pt[i] := longint(new_page) or USER_PAGE;
      end;
